...
class ModelHMD3D_Binocular(Model3D_Binocular):
    ...
    def _fit(self, X, Y):
        ...
        logger.info("about to calibrate HMD inside ModelHMD3D_Binocular")
...
class PosthocModelHMD3D_Binocular(Model3D_Binocular):
    def _fit(self, X, Y):
        ...
        logger.info("Running PosthocModelHMD3D_Binocular.fit()")

        print("Running PosthocModelHMD3D_Binocular.fit()")
...
class PosthocGazerHMD3D(Gazer3D):
    ...
    logger.info("Running PosthocGazerHMD3D")
    ...
    def fit_on_calib_data(self, calib_data):
        ...
        if not self.posthoc_calib:
            ...
            #ref_data = calib_data["ref_list"]
            ref_data = calib_data["ref_list"]
            self.g_pool.realtime_ref = calib_data["ref_list"]

            # # file_path = Path(rt_reference_path).expanduser()
            # from pathlib import Path
            # import msgpack
            # import gc
            # file_path = self.g_pool.rec_dir + "\\" + "realtime_calib_points.msgpack"
            # with Path(file_path).expanduser().open("rb") as fh:
            #     gc.disable()  # speeds deserialization up.
            #     realtime_ref = msgpack.unpack(fh, strict_map_key=False)

            # self.g_pool.realtime_ref = realtime_ref['data']
